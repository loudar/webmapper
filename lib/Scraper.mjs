    ...
    async getPageSelenium(host, url) {
        try {
            ...
            if (preRes.headers['content-type']) {
                if (preRes.headers['content-type'].startsWith("text")) {
                    contentType = "text";
                }
                if (preRes.headers['content-type'].startsWith("image")) {
                    contentType = "image";
                }
                if (preRes.headers['content-type'].startsWith("video")) {
                    contentType = "video";
                }
                if (preRes.headers['content-type'].startsWith("audio")) {
                    contentType = "audio";
                }
                if (preRes.headers['content-type'].startsWith("application/pdf")) {
                    contentType = "pdf";
                }
                if (preRes.headers['content-type'].startsWith("application/zip")) {
                    contentType = "zip";
                }
                if (preRes.headers['content-type'].startsWith("application/vnd.openxmlformats-officedocument.wordprocessingml.document")) {
                    contentType = "docx";
                }
                if (preRes.headers['content-type'].startsWith("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")) {
                    contentType = "xlsx";
                }
                if (preRes.headers['content-type'].startsWith("application/vnd.openxmlformats-officedocument.presentationml.presentation")) {
                    contentType = "pptx";
                }
            }
