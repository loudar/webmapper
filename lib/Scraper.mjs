...
export class Scraper {
    ...
    static async scrapeSites(db, scraper, startingUrl, onlyNew) {
        ...
        while (urls.length > 0) {
            ...
            await this.processBatch(semaphore, linksSemaphore, total, urls, scrapedUrls, db, scraper);
            addToQueue = await db.getUnprocessedLinks();
    ...
    static async processBatch(semaphore, linksSemaphore, total, urls, scrapedUrls, db, scraper) {
        let jobs = [];
        let i = 0;
        for (const url of urls) {
            ...
            const func = async () => {
                ...
                const info = await this.processUrl(url, linksSemaphore, scrapedUrls, db, scraper);
    ...
    static async processUrl(url, linksSemaphore, scrapedUrls, db, scraper) {
