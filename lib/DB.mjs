...
export class DB {
    ...
    getUnprocessedLinks() {
        return this.query("SELECT * FROM links WHERE status IS NULL OR (status IS NOT NULL AND status > 0 AND id NOT IN (SELECT origin_id FROM links_linked))");
    }
...
ORDER BY
    ...
    async getClustersWithoutSubdomains() {
        const rows = await this.query(`SELECT
        SUBSTRING_INDEX(links.host, '.', -2) as domain,
        COUNT(ll.target_id) AS outgoing_link_count,
        GROUP_CONCAT(DISTINCT CONCAT(SUBSTRING_INDEX(l2.host, '.', -2), '(', coalesce(l2.incoming_link_count, 0), ')')) AS target_domains
    FROM
        links
    LEFT JOIN
        links_linked ll ON links.id = ll.origin_id
