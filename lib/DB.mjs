...
AND status = 200`);
    ...
    async insertUser(username, hashedPassword) {
        await this.query("INSERT INTO users (username, password) VALUES (?, ?)", [username, hashedPassword]);
    }

    async insertSearch(id, query) {
        if (query.length < 3) return;
        await this.query("INSERT INTO search_history (user_id, query) VALUES (?, ?)", [id, query]);
    }
