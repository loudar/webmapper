...
export class DB {
    ...
    async insertLink(link, status, content) {
        ...
        try {
            console.log(`Inserting link ${link} with content (${content.length}) and status ${status}...`);
            await this.query("INSERT INTO links (link, status, content, host) VALUES (?, ?, ?, ?)", [link, status, content, host]);
            console.log(`Done inserting link ${link} with host ${host}.`);
        } catch (e) {
            if (e.code === "ER_DUP_ENTRY") {
                try {
                    console.log(`Updating link ${link} with content (${content.length}) and status ${status}...`);
    ...
    getUnprocessedLinks(size, excludedTerms) {
        const excludeQuery = excludedTerms.map(() => "link NOT LIKE ?").join(" AND ");
        return this.query(`SELECT * FROM links WHERE status IS NULL AND (${excludeQuery}) LIMIT ?`,
