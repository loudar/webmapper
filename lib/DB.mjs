...
export class DB {
    ...
    async handleInsertBatch(batch) {
        const sql = "INSERT INTO links (link, host) VALUES " + batch.map(() => "(?, ?)").join(", ") + " ON DUPLICATE KEY UPDATE link = link";
        const params = [];
    ...
    async searchContentExplicit(query) {
        const searchTerm = `%${query.replace(/\s/g,"\\s")}%`;
        const searchNoLike = query;
        const previewSize = 100;
        const startOffset = previewSize / 2;
        return await this.query(`SELECT
    link, SUBSTRING(content, GREATEST(1, LOCATE(?, content) - ?), LENGTH(?) + ?) AS preview
FROM
    links
WHERE
    content LIKE BINARY ? LIMIT 10000`, [searchNoLike, startOffset, searchNoLike, previewSize, searchTerm]);
    }
...
FROM
    links
WHERE
    link LIKE BINARY ? LIMIT 10000`, [searchNoLike, startOffset, searchNoLike, previewSize, searchTerm]);
    }
}